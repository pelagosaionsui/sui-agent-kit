...
import { z } from "zod";
import { tradeByCetus } from "../../tools";

const tradeByCetusAction: Action = {
  name: "TRADE_BY_CETUS",
  similes: [
    "swap tokens",
    "exchange tokens",
  ...
  handler: async (agent: SuiAgentKit, input: Record<string, any>) => {
    const tx = await tradeByCetus(agent, input.target, input.amount, input.from);
...
export default tradeByCetusAction;