...
export class PythEMAPriceTool extends Tool {
    name = "pyth_ema_price";
    description = `Use this tool to fetch the real-time EMA price of a coin from Pyth by ticker.
    ...
    protected async _call(input: string): Promise<string> {
      try {
        ...
        const price = await this.suiAgentKit.fetchPythEMAPrice(priceFeedID);

        return JSON.stringify({
          status: "success",
          message: `EMA price fetched successfully`,
          price: price,
        });
      } catch (error: any) {
        return JSON.stringify({
